...
# Save graphs
pdf("Raw_results/PR010_network_means_PCC.pdf", height = 5, width = 8)
pcc_m <- plot(NetMean, "PCC", labels = names.means, edge.labels = TRUE, layout = "spring")
dev.off()

pdf("Raw_results/PR010_network_means_PDC.pdf", height = 5, width = 8)
pdc_m <- plot(NetMean, "PDC", labels = names.means)
dev.off()

pdf("Raw_results/PR010_networks_means.pdf", height = 5, width = 8)
plot(NetMean, 
     labels = names.means, 
     edge.labels = TRUE, 
     layout="spring")
...
ggsave(PR010_centplotm, filename = "Raw_results/PR010_centrality_means.png")

# centrality tables
centr_m <- clairecleans::gather_cent(pcc_m, pdc_m)
rm(centr)
write.csv(centr_m, "Raw_results/PR010_centr_means.csv", row.names = FALSE)

# node correlations. Get edge list and weights
(pcc_corr_m <- data.frame(pcc_m[[1]][[2]][[6]]))
write.csv(pcc_corr_m, "Raw_results/PR010_pcc_corr_means.csv", row.names = TRUE)

(pdc_corr_m <- data.frame(pdc_m[[1]][[2]][[5]]))
write.csv(pdc_corr_m, "Raw_results/PR010_pdc_corr_means.csv", row.names = TRUE)
```
...
# save graphs
pdf("Raw_results/PR010_network_sd_PCC.pdf", height = 5, width = 8)
pcc_sd <- plot(NetSD, "PCC", labels = names.sd, edge.labels = TRUE, layout="spring")
dev.off()

pdf("Raw_results/PR010_network_sd_PDC.pdf", height = 5, width = 8)
pdc_sd <- plot(NetSD, "PDC", labels = names.sd)
dev.off()

pdf("Raw_results/PR010_networks_sd.pdf", height = 5, width = 8)
...
PR010_centplotsd <- plot(NetSD, labels = names.sd, 
  ...
  theme(axis.text=element_text(size=13),
        strip.text.y = element_text(size=15))
ggsave(PR010_centplotsd, filename = "Raw_results/PR010_centrality_sd.png")

# centrality tables
centr_sd <- clairecleans::gather_cent(pcc_sd, pdc_sd)
rm(centr)
write.csv(centr_sd, "Raw_results/PR010_centr_sd.csv", row.names = FALSE)

# node correlations
pcc_corr_sd <- data.frame(pcc_sd[[1]][[2]][[6]])
pcc_corr_sd # look at it
write.csv(pcc_corr_sd, "Raw_results/PR010_pcc_corr_sd.csv", row.names = TRUE)

pdc_corr_sd <- data.frame(pdc_sd[[1]][[2]][[5]])
pdc_corr_sd # look at it
write.csv(pdc_corr_sd, "Raw_results/PR010_pdc_corr_sd.csv", row.names = TRUE)
```
...
# Save graphs
pdf("Imputed_results/PR010_network_means_PCC_imputed.pdf", height = 5, width = 8)
pcc_m <- plot(NetMean, "PCC", labels = names.means, edge.labels = TRUE, layout = "spring")
dev.off()

pdf("Imputed_results/PR010_network_means_PDC_imputed.pdf", height = 5, width = 8)
pdc_m <- plot(NetMean, "PDC", labels = names.means)
dev.off()

pdf("Imputed_results/PR010_networks_means_imputed.pdf", height = 5, width = 8)
plot(NetMean, 
     labels = names.means, 
     edge.labels = TRUE, 
     layout="spring")
...
PR010_centplotm <- plot(NetMean, 
  ...
  theme(axis.text = element_text(size = 13),
        strip.text.y = element_text(size = 15))
ggsave(PR010_centplotm, filename = "Imputed_results/PR010_centrality_means_imputed.png")

# centrality tables
centr_m <- clairecleans::gather_cent(pcc_m, pdc_m)
rm(centr)
write.csv(centr_m, "Imputed_results/PR010_centr_means_imputed.csv", row.names = FALSE)

# node correlations. Get edge list and weights
(pcc_corr_m <- data.frame(pcc_m[[1]][[2]][[6]]))
write.csv(pcc_corr_m, "Imputed_results/PR010_pcc_corr_means_imputed.csv", row.names = TRUE)

(pdc_corr_m <- data.frame(pdc_m[[1]][[2]][[5]]))
write.csv(pdc_corr_m, "Imputed_results/PR010_pdc_corr_means_imputed.csv", row.names = TRUE)
```
...
# save graphs
pdf("Imputed_results/PR010_network_sd_PCC_imputed.pdf", height = 5, width = 8)
pcc_sd <- plot(NetSD, "PCC", labels = names.sd, edge.labels = TRUE, layout="spring")
dev.off()

pdf("Imputed_results/PR010_network_sd_PDC_imputed.pdf", height = 5, width = 8)
pdc_sd <- plot(NetSD, "PDC", labels = names.sd)
dev.off()

pdf("Imputed_results/PR010_networks_sd_imputed.pdf", height = 5, width = 8)
PR010_centplotsd <- plot(NetSD, labels = names.sd, 
                         edge.labels = TRUE, 
                         layout = "spring")
dev.off()
...
PR010_centplotsd <- plot(NetSD, labels = names.sd, 
  ...
  theme(axis.text=element_text(size=13),
        strip.text.y = element_text(size=15))
ggsave(PR010_centplotsd, filename = "Imputed_results/PR010_centrality_sd_imputed.png")

# centrality tables
centr_sd <- clairecleans::gather_cent(pcc_sd, pdc_sd)
rm(centr)
write.csv(centr_sd, "Imputed_results/PR010_centr_sd_imputed.csv", row.names = FALSE)

# node correlations
pcc_corr_sd <- data.frame(pcc_sd[[1]][[2]][[6]])
pcc_corr_sd # look at it
write.csv(pcc_corr_sd, "Imputed_results/PR010_pcc_corr_sd_imputed.csv", row.names = TRUE)

pdc_corr_sd <- data.frame(pdc_sd[[1]][[2]][[5]])
pdc_corr_sd # look at it
write.csv(pdc_corr_sd, "Imputed_results/PR010_pdc_corr_sd_imputed.csv", row.names = TRUE)
```
...
# # PR010: not detrended for neans or SDs; imputed and non-imputed
```

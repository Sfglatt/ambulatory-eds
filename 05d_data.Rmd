...
# EMA data
```{r EMA}
setwd("Raw_data\\05d_raw_data\\Raw_EMA")
...
```{r baseline}
baseline <- read.csv("Raw_data/05d_raw_data/R15 baseline.csv")
View(baseline)
...
baseline <- baseline %>%
  mutate(participant_id = recode(participant_id, 
                                    ...
                                    "pr231" = "PR231",
                                    "pr074" = "PR074", 
                                    "Pr248" = "PR248")) # when merging baseline with EMA - found that some ID's were missing since they were labeled incorrectly.
...
baseline$oci_sum <- rowSums(as.data.frame(lapply(baseline[, paste0("oci_", 1:18)], function(x)
  as.numeric(gsub("\\s", "", x)))),
  na.rm = TRUE)
...
write.csv(dat, 
          file = paste0("05d_Output/Created_data/EMA_", format(Sys.Date(), "%Y_%m_%d"), ".csv"), 
          row.names = TRUE) # version *with* empty rows (no OCI filter)

write.csv(dat_oci_filter, 
          file = paste0("05d_Output/Created_data/EMA_OCI21_", format(Sys.Date(), "%Y_%m_%d"), ".csv"), 
          row.names = TRUE) # version *with* empty rows and OCI >= 21

write.csv(dat_clean, 
          file = paste0("05d_Output/Created_data/EMA_clean_", format(Sys.Date(), "%Y_%m_%d"), ".csv"), 
          row.names = TRUE) # version *without* empty rows (no OCI filter)

write.csv(dat_clean_oci_filter, 
          file = paste0("05d_Output/Created_data/EMA_OCI21_clean_", format(Sys.Date(), "%Y_%m_%d"), ".csv"), 
          row.names = TRUE) # version *without* empty rows and OCI >= 21

dat_clean_oci_filter %>%
  split(.$Participant.Label) %>%
  walk2(names(.), ~ write.csv(.x, file.path("05d_Output/Created_data/EMA_OCI21_clean/",
                                            paste0("EMA_", .y, ".csv")),
                              row.names = TRUE)) # person-level CSVs *without* empty rows and OCI >= 21
```
...
write.csv(dat_oci_filtered_IV, 
          file = paste0("05d_Output/Created_data/EMA_OCI21_vars_", format(Sys.Date(), "%Y_%m_%d"), ".csv"), 
          row.names = TRUE) 
```
